---
name: tkk-case
description: 法律案件全流程处理技能。支持任意方式启动（口述/粘贴/文件）→ 与用户沟通厘清案件 → 调用 tkk-legal-ingest 分析问题（如无答案则用 web-access 检索）→ 生成结构化法律分析文章 → 生成配图 → 存入vault问答目录。触发场景：用户说"分析这个案子"、"生成法律文章"、"整理案例分析"、"我想咨询一个案件"。
---
...
2. 通过提问逐步厘清案件：涉及人员、行为模式、金额、时间节点等
3. 必要时引导用户提供关键信息

**沟通原则**：
- 像咨询顾问一样引导对话
- 不要求用户一次性提供完整素材
- 根据法律分析需要，逐步向用户确认关键事实

---

### Step 1 — 接收并解析素材（文件模式下）
...
**根据文章段落内容选择配图**：不预设张数和固定类型，而是分析文章各段落的内容，选取最适合用图像表达的核心论点。

**选择逻辑**：
1. 阅读文章各段落的论点
2. 识别适合可视化的内容（如：时间线、对比结构、法律流程、金额数据、关系图谱等）
3. 为每个候选段落生成一张配图
4. 配图数量由文章内容决定，通常 2-5 张
...
**配图 prompt 原则**：
- 提取段落的核心论点，用一句话概括
- prompt 应包含"Chinese text"和核心内容描述
- 风格简洁专业，法律主题
...
~/.baoyu-skills/baoyu-imagine/run-with-env.sh \
  --prompt "Professional legal infographic, dark blue background, Chinese text '核心论点一句话概括', key visual elements" \
  --image "8输出/tkk-case/images/{案件关键词}-{序号}.jpg" \
...
在文章适当位置插入 markdown 图片引用，基于配图与段落内容的对应关系：
...
| v6 | 2026-04-27 | case_file改为选填；新增Step 0沟通启动；入库路径改为8输出/tkk-case |
